...
options(width = 200) # increase page width for wide readouts
...
<center>
<iframe src="https://macaulaylibrary.org/asset/643030692/embed" height="476" width="640" frameborder="0" allowfullscreen></iframe>
</center>
...
```{r graph linear model top predictors, echo=FALSE, fig.width=10, fig.height=5, fig.align='center', message=FALSE}
p1 <- ggplot(combined.detections, aes(x = min_from_sunrise, y = total_individuals)) +
  geom_point() +
  geom_smooth(method="glm", method.args=list(family="poisson"(link="log"))) +
  xlab("Minutes from Sunrise") + ylab("Total Number of Detections") + 
  theme_classic()
p2 <- ggplot(combined.detections, aes(x = pc_grass, y = total_individuals)) +
  geom_point() +
  geom_smooth(method="glm", method.args=list(family="poisson"(link="log"))) +
  xlab("Percent Grass") + ylab("Total Number of Detections") +
  theme_classic()
p3 <- ggplot(combined.detections, aes(x = vd_vo, y = total_individuals)) +
  geom_point() +
  geom_smooth(method="glm", method.args=list(family="poisson"(link="log"))) +
  xlab("Vegetation Density") + ylab("Total Number of Detections") + 
  theme_classic()
...
Now let's see what happens if we add site id as a random effect since there were multiple point counts per site. 

```{r model list mixed models, include=FALSE}
m13 <- glmer(total_individuals ~ veg_ht_avg + pc_grass + (1|site_id), family = poisson(link = "log"), data = combined.detections)
m2 <- glm(total_individuals ~ site_ha + pc_grass, family = poisson(link = "log"), data = combined.detections)
m3 <- glm(total_individuals ~ site_ha + min_from_sunrise, family = poisson(link = "log"), data = combined.detections)
m4 <- glm(total_individuals ~ litter_avg + veg_ht_avg + pc_grass, family = poisson(link = "log"), data = combined.detections)
m5 <- glm(total_individuals ~ vd_vo + pc_grass + min_from_sunrise, family = poisson(link = "log"), data = combined.detections)
